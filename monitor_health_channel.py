#!/usr/bin/env python3
"""
Continuous monitoring of 養生之道健康長壽 channel download progress.
"""
...
def monitor_continuous():
    ...
    print("📊 養生之道健康長壽 Channel Download Monitor")
    print("="*60)
    ...
    while True:
        try:
            if log_file.exists():
                ...
                # Count completed
                completed = len(re.findall(r'✅ \[(\d+)/20\] Completed', content))
                
                # Count started
                started_matches = re.findall(r'\[(\d+)/20\] Starting download', content)
                ...
                last_operations = re.findall(r'(✅ \[\d+/20\].*|🎬 \[\d+/20\].*|Successfully extracted.*)', content)[-3:]
                
                # Build status
                progress_pct = (completed / 20) * 100
                status = f"Progress: {completed}/20 ({progress_pct:.1f}%) | Started: {started} | "
                ...
                # Check if complete
                if completed >= 20:
                    print(f"\n\n🎉 All 20 videos downloaded successfully!")
            ...
            print(f"Final status: {completed}/20 completed")
            break
    ...
    if log_file.exists():
        ...
        # Try to find the channel directory (might have different ID)
        ...
        for channel_dir in channel_dirs:
            # Check if this is the right channel by looking for recent files
            video_dirs = [d for d in channel_dir.iterdir() if d.is_dir() and not d.name.startswith('.')]
            
            for video_dir in video_dirs[-20:]:  # Check last 20 videos
